"""
Seamless Fabric Tiling Pipeline #5
=================================
...
# V5 tile-selection changes:
# - FFT uses min_period + peak quality checks instead of a fixed 100 px floor.
# - TexTile scores VGG/FFT period candidates, but does not create tile sizes.
# - Crop offsets are searched over the full image with a local refinement pass.
# - Area is a small relative-area regularizer.
...
def print_progress(label, done, total, start_time):
    ...
    print(f"[progress] {label}: {done}/{total} ({percent:5.1f}%) elapsed={elapsed}")
...
def validate_cli_paths(args):
    """입출력 경로를 실행 초기에 검증하고 문제를 모아 반환합니다."""
    errors = []
    image_exts = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".webp"}

    input_path = str(args.input).strip()
    if not input_path:
        errors.append("--input path is empty.")
    elif not os.path.isfile(input_path):
        errors.append(f"--input file not found: {args.input}")

    for arg_name, value in (
        ("--output", args.output),
        ("--patch-output", args.patch_output),
    ):
        path_value = str(value).strip()
        if not path_value:
            errors.append(f"{arg_name} path is empty.")
            continue

        ext = os.path.splitext(path_value)[1].lower()
        if ext not in image_exts:
            errors.append(
                f"{arg_name} must include a valid image extension "
                f"({', '.join(sorted(image_exts))}): {value}"
            )
        ...
        if parent:
            try:
                ...
                errors.append(
                    f"{arg_name} directory is invalid or not writable: {parent} ({exc})"
                )

    candidate_dir = str(args.candidate_output_dir).strip()
    if not candidate_dir:
        errors.append("--candidate-output-dir path is empty.")
    else:
        if os.path.exists(candidate_dir) and not os.path.isdir(candidate_dir):
            errors.append(
                "--candidate-output-dir points to a file, not a directory: "
                f"{args.candidate_output_dir}"
            )
        else:
            try:
                ...
                errors.append(
                    "--candidate-output-dir is invalid or not writable: "
                    f"{candidate_dir} ({exc})"
                )
...
def _dominant_period_peak(profile, lo, hi, min_prominence=0.03):
    lo = max(1, int(lo))
    hi = min(int(hi), profile.numel() - 2)
    if hi <= lo:
        return None
    ...
    left = profile[lo - 1 : hi]
    right = profile[lo + 1 : hi + 2]
    local_max = (segment >= left) & (segment >= right)
    baseline = segment.median()
    strong = segment > baseline + float(min_prominence)
    candidate_mask = local_max & strong
    ...
    candidate_scores = torch.where(
        candidate_mask,
        segment,
        segment.new_full(segment.shape, -float("inf")),
    )
    return int(torch.argmax(candidate_scores).item()) + lo


@torch.no_grad()
def estimate_period_fft_power(
    img,
    min_period=8,
    max_period=None,
    min_prominence=0.03,
    ...
    # Use a relative lower bound and reject weak peaks instead of forcing 100 px.
    dynamic_min = max(int(min_period), max(4, min(img_h, img_w) // 64))
    max_p_h = min(int(max_period or img_h // 2), img_h - 1)
    max_p_w = min(int(max_period or img_w // 2), img_w - 1)
    py = _dominant_period_peak(ac[:, 0], dynamic_min, max_p_h, min_prominence)
    px = _dominant_period_peak(ac[0, :], dynamic_min, max_p_w, min_prominence)
    if py is None or px is None:
        return None

    # 너무 작은 주기를 방지하기 위한 하한.
    # 단, 이미지가 더 작다면 해당 축의 최대 크기로 제한합니다.
    py = int(max(1, min(py, img_h)))
    px = int(max(1, min(px, img_w)))
    return py, px
...
def build_patch_candidates(periods, image_hw, max_multiple=3):
    ...
    def add_candidate(source, size, period=None, multiple=1):
        patch_h = int(size[0])
        patch_w = int(size[1])
        if patch_h < 1 or patch_w < 1 or patch_h > img_h or patch_w > img_w:
            return
        key = (patch_h, patch_w)
        if key in by_size:
    ...
    for source, period in periods:
        if period is None:
            continue
        base_h, base_w = int(period[0]), int(period[1])
        for multiple in range(1, max(1, int(max_multiple)) + 1):
            size = (base_h * multiple, base_w * multiple)    #뽑아낸 주기를 Scaling (max_multiple) 배수까지 확대
            label = source if multiple == 1 else f"{source}x{multiple}"
            add_candidate(label, size, period=period, multiple=multiple)
...
class TileabilityLoss:
    ...
    def __init__(
        ...
        if image_hw is not None:
            self.image_area = max(1, int(image_hw[0]) * int(image_hw[1]))
    ...
    def score_parts(self, img):
        ...
        # Small regularizer: prefer smaller tiles only when quality is similar.
        area_loss = area / float(self.image_area or max(1, area))
        total = (
...
def select_patch_at_size(
    ...
    max_oy = max(0, img_h - patch_h)
    max_ox = max(0, img_w - patch_w)
    coarse_step = max(1, int(offset_step))

    def axis_offsets(limit, step):
        values = list(range(0, limit + 1, step))
        if not values or values[-1] != limit:
            values.append(limit)
        return values

    def evaluate_grid(y_values, x_values, best=None, label=None):
        total = max(1, len(y_values) * len(x_values))
        ...
        for oy in y_values:
            for ox in x_values:
                ...
                if label and (done == 1 or done == total or done % interval == 0):
                    print_progress(label, done, total, start_time)
        return best

    # Search the whole image first, then refine locally around the best offset.
    best = evaluate_grid(
        axis_offsets(max_oy, coarse_step),
        axis_offsets(max_ox, coarse_step),
        label=f"{progress_label} offset coarse" if progress_label else None,
    )

    if coarse_step > 1:
        _, best_y, best_x, _ = best
        radius = int(refine_radius or coarse_step * 2)
        fine_step = max(1, int(refine_step))
        y0 = max(0, best_y - radius)
        y1 = min(max_oy, best_y + radius)
        x0 = max(0, best_x - radius)
        x1 = min(max_ox, best_x + radius)

        y_values = [y0 + value for value in axis_offsets(y1 - y0, fine_step)]
        x_values = [x0 + value for value in axis_offsets(x1 - x0, fine_step)]
        local_best = evaluate_grid(
            y_values,
            x_values,
            best=None,
            label=f"{progress_label} offset refine" if progress_label else None,
        )
...
def extract_tile_patch(
    ...
    """VGG/FFT 주기 후보와 TexTile 크기 후보를 결합해 최적 패치를 추출합니다. VGG, FFT 각 한개씩 하면 후보 2개, TexTile로 multiple 3배까지 -> 후보 6개"""
    img_h, img_w = img.shape[-2:]

    # Step 1) 특성 기반 구조(VGG)와 주파수 기반 구조(FFT)에서 주기 후보를 수집합니다.
    feat = act_fn(img)[0]
    py_vgg, px_vgg, _, _ = _period_from_feature(
        feat,
        act_fn.stride,
    ...
    fft_period = None
    if use_fft:
        fft_period = estimate_period_fft_power(
            img,
            min_period=min_period,
            max_period=max_period,
        )
        if fft_period is not None:
            periods.append(("fft", fft_period))

    # Step 2) TexTile 점수 기반 크롭 크기 후보를 추가합니다.
    # TexTile scores candidates only; it no longer creates independent sizes.
    textile_sizes = []
    ...
    candidates = build_patch_candidates(
        periods,
        (img_h, img_w),
        max_multiple=max_tile_multiple,
    )
    ...
    for idx, candidate in enumerate(candidates, start=1):
        if progress:
            print_progress("candidate patches", idx, candidate_total, candidate_start)
        patch, crop, parts = select_patch_at_size(
            img,
            candidate["size"],
            patch_scorer,
            offset_step=offset_step,
            progress_label=f"candidate {idx}/{candidate_total}" if progress else None,
        )
    ...
    info = {
        ...
        "fft_period": fft_period if use_fft else None,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Minimum period used by VGG/FFT period search.",
    )
    parser.add_argument(
        "--max-period",
        type=int,
        default=None,
        help="Maximum period used by VGG/FFT period search.",
    )
    parser.add_argument(
        "--max-tile-multiple",
        type=int,
        default=3,
        help="Keep base period candidates and their integer multiples up to this value.",
    )
    parser.add_argument(
        "--textile-ckpt",
    ...
    parser.add_argument(
        "--no-textile",
        action="store_true",
        help="Disable TexTile scoring and rotation search.",
    )
    parser.add_argument(
        "--textile-resolution",
        type=parse_size,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Deprecated in v5; TexTile no longer creates independent size candidates.",
    )
    parser.add_argument(
        "--textile-size-top-k",
        type=int,
        default=4,
        help="Deprecated in v5; kept only for CLI compatibility.",
    )
    parser.add_argument(
        "--rotation-step",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Small weight for relative patch-area regularization.",
    )
    parser.add_argument(
        "--style-resolution",
        type=parse_size,
    ...
    parser.add_argument(
        "--no-progress",
        action="store_true",
        help="Disable progress and elapsed-time logging.",
    )
    args = parser.parse_args()

    path_errors = validate_cli_paths(args)
    if path_errors:
        print("[error] Invalid path arguments:")
    ...
    patch, (oy, ox, patch_h, patch_w), extract_info = extract_tile_patch(
        ...
        max_tile_multiple=args.max_tile_multiple,
        progress=show_progress,
    )
    ...
    print(f"[done] total elapsed={format_elapsed(time.perf_counter() - run_start)}")
